...
//Added stuff for decryption
#include "EncryptionEnclave_u.h"
#define ENCLAVE_NAME "EncryptionEnclave/encryptionenclave.signed.so"
...
size_t b64passwortLen;
size_t *b64passwortLenPointer;
char * b64passwordPointer;
size_t myDecMessageLen;
char *myDecMessage;

char* receiveMyMessage(MsgIO *msgio,char* input, const char* myType){
	char*receivedData;

	//Marker1:
	//Added for Prototype: Receive additional messages before end
	size_t receivedDataSize;
	int rv_1	;
	rv_1= msgio->read((void **) &receivedData, &receivedDataSize);
	if ( rv_1 == -1 ) {
		eprintf("system error reading receivedData\n");
		return receivedData;
	} else if ( rv_1 == 0 ) {
		eprintf("protocol error reading receivedData\n");
		return receivedData;
	}
	fprintf(stderr, "Received message of Type %s: \n   Text: %s \n   Length: %i\n", myType, receivedData, receivedDataSize);
	//*input = *receivedData;
	sleep(1);
	char* outgoingStuff = "OK";	
	msgio->send(outgoingStuff, 3);
...
sgx_status_t statusEnclaveCall, statusEncryption, ret;
char *password = "FromOutside";
char buffer[100] = "Hello World!";
sgx_sha256_hash_t mkPwHash;
		int i = 0;
...
void ocall_print_string(const char *str)
{
     ...
     */
    printf("ocall_print_string: %s\n", str);
}
void ocall_print_hexkey(const char *str)
{
    /* Proxy/Bridge will check the length and null-terminate 
     * the input string to prevent buffer overflow. 
     */
    printf("ocall_print_hexkey: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X \n",
	 str[0],str[1],str[2],str[3],str[4],str[5],str[6],str[7],str[8],str[9],str[10],str[11],str[12],str[13],str[14],str[15]);
  //  printf("ocall_print_string: 0x%02X\n", key[1]);
...
char *myDecMessage2 = "test";
size_t b64passwortLenPointer2 = 1;
char * b64passwordPointer2 = "test";
	const char *myMessage3 ="testweise";


	char* MyStuff="TEST-UTPU---------------------";
	char* MyStuff2[1000];
	int myRetVal = 0;
	size_t output;
	size_t output2;
	...
	char * outgoingStuff = "OK";
	char* MyStuff3="TEST-UTPU---------------------";
	char* MyStuff4="TEST-UTPU---------------------";
	size_t output3;
	size_t output4;
	sgx_status_t enclaveStatus;
	enclaveStatus = sgx_create_enclave(ENCLAVE_NAME, SGX_DEBUG_FLAG, &token, &updated, &eid, 0);
	 ...
	 */
		...
		ret = setEncryptionKey(eid, (const char*) &key);
		if (ret != SGX_SUCCESS) {
			fprintf(stderr, "Error: Setting encryption key with mk: %08x\n", ret);
		}
	...
	//Added for Prototype: Receive additional messages before end
	size_t * testz;
	myMessage= receiveMyMessage(msgio,myMessage, "ConnectorType");
	printf("TESTMESSAGE...%s\n", myMessage);
	myMessage= receiveMyMessage(msgio,myMessage, "InfoModel");
	printf("TESTMESSAGE...%s\n", myMessage);
	myMessage= receiveMyMessage(msgio,myMessage, "UserCredentials");
	printf("TESTMESSAGE...%s\n", myMessage);
	myMessage= receiveMyMessage(msgio,myMessage, "RawPWData");
	printf("TESTMESSAGE...%s\n", myMessage);

	printf("base64 decoding1...\n");
		

	myMessage= base64_decode(myMessage, &sigsz);
	if ( myMessage == NULL ) {
	...
	while (i < sigsz)
	{
		printf(" %i-%02x",i, myMessage[i]);
		i++;
	}
	printf("\nDEBUG: Sizeof encrypted Message: %i\n", (int) sigsz);

	myMessageDec = (char *)malloc((14 + 1) * sizeof(char));
	printf("Decrypting...\n");
	ret = decryptMessage(eid, myMessage, sigsz, myMessageDec, (size_t) 14);
	if (ret != SGX_SUCCESS) {
	/*	enclave_ra_close(eid, &sgxrv, ra_ctx);
		fprintf(stderr, "Error: Decrypting password with mk: %08x\n", status);
		fprintf(fplog, "Error: Decrypting password with mk: %08x\n", status);
	...
	printf("Decrypted messageWARNING\n");
...
/* Windows implementation is not available yet */
		...
		//Marker End of Attestation process.
		...
		edividerWithText("Starting SGX Connector Communication"); 

		if ( msg4->status == Trusted || true ) {
			unsigned char hashmk[32], hashsk[32];

			if ( true ) eprintf("+++ Deriving the MK and SK\n");
		...
		ret = setEncryptionKey(eid, (const char*) session->mk);
		if (ret != SGX_SUCCESS) {
			fprintf(stderr, "Error: Setting encryption key with mk: %08x\n", ret);
		}
//		getchar();
