...
sgx_status_t ret, global_ret;

extern std::map<sgx_enclave_id_t, uint32_t>g_enclave_id_map;
extern uint32_t enclave_temp_no;
...
//extern sgx_enclave_id_t e1_enclave_id = 0;
//extern sgx_enclave_id_t e2_enclave_id = 0; TODO cleanup

#include "UntrustedEnclaveMessageExchange.h"

//From Intel SGX SDK Sample Apps LocalAttestationApp-Code
...
int main (int argc, char *argv[])
{
...
#ifdef _WIN32
	status = sgx_create_enclave(MANAGEMENT_ENCLAVE, SGX_DEBUG_FLAG,
		&token, &updated, &e1_enclave_id, 0);
	if (status != SGX_SUCCESS) {
		fprintf(stderr, "sgx_create_enclave: %s: %08x\n",
			MANAGEMENT_ENCLAVE, status);
		return 1;
	}
#else
	status = sgx_create_enclave_search(MANAGEMENT_ENCLAVE,
		SGX_DEBUG_FLAG, &token, &updated, &e1_enclave_id, 0);
	if ( status != SGX_SUCCESS ) {
		fprintf(stderr, "sgx_create_enclave1: %s: %08x\n",
			MANAGEMENT_ENCLAVE, status);
	...
	//After creation of ManagementEnclave add its eid to map of enclaves:
	add_enclave_to_map_wrapper(e1_enclave_id);
	
	//Test hardcoded sample data savings
	if (save_testdata_in_enclave(&e1_enclave_id) != 0) printf("\nERROR:Save_testdata_in_enclave failed!\n");
	get_number_of_votes_wrapper(e1_enclave_id);
	
	//Test manually added sample data savings
	char testData[5][MAX_BUF_LEN] = { "ID3", "Peterella", "Frau", "Trump", "RL" };
	if (save_data_in_enclave(&e1_enclave_id, testData) != 0) printf("\nERROR:Save_testdata_in_enclave failed!\n");
	get_number_of_votes_wrapper(e1_enclave_id);

	fprintf(stderr, "CHECKING POLICY\n\n\n\n");

	char* rights = "TO";
	int resultPolicyGeneration = generate_policy(rights);

	//Init OutputEnclave

	fprintf(stderr, "CHECKING CHRONO\n\n\n\n");
	//saving to statistics.txt
	ofstream fout;
	fout.open("statistics_initOutputEnclave.txt", std::ios::app);
    std::chrono::time_point<std::chrono::system_clock> start, end; 
  
    start = std::chrono::high_resolution_clock::now(); 
	int updated_OutputEnclave; sgx_launch_token_t token_OutputEnclave;
	if (sgx_create_enclave(OUTPUT_ENCLAVE, SGX_DEBUG_FLAG, &token_OutputEnclave, &updated_OutputEnclave, &e2_enclave_id, 0) != SGX_SUCCESS) 
		printf("Error loading enclave");
    end = std::chrono::high_resolution_clock::now(); 

    std::chrono::duration<double> elapsed_seconds = end - start; 
    std::time_t end_time = std::chrono::high_resolution_clock::to_time_t(end);   
    std::cout << "Init OutputEnclave: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout << "Init OutputEnclave: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout.close();

	add_enclave_to_map_wrapper(e2_enclave_id);


	//saving to statistics.txt
	ofstream fout4;
	fout4.open("statistics_initEnc2Enc.txt", std::ios::app);
  
    start = std::chrono::high_resolution_clock::now(); 
	test_crate_session_wrapper(e1_enclave_id, e2_enclave_id);
    end = std::chrono::high_resolution_clock::now(); 

	elapsed_seconds = end - start; 
    end_time = std::chrono::high_resolution_clock::to_time_t(end);   
    std::cout << "Init Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout4 << "Init Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout4.close();

	test_message_exchange_wrapper(e1_enclave_id, e2_enclave_id);
	test_enclave_to_enclave_call_wrapper(e1_enclave_id, e2_enclave_id);

	//saving to statistics.txt
	ofstream fout5;
	fout5.open("statistics_copyEnc2Enc.txt", std::ios::app);
  
    start = std::chrono::high_resolution_clock::now(); 
	test_copy_all_data_enclave_to_enclave_call_wrapper(e1_enclave_id, e2_enclave_id);
    end = std::chrono::high_resolution_clock::now(); 

    elapsed_seconds = end - start; 
    end_time = std::chrono::high_resolution_clock::to_time_t(end);   
    std::cout << "Copy Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout5 << "Copy Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout5.close();

	//Code for comparison of normal voting copying command
	//saving to statistics.txt
	ofstream fout6;
	fout6.open("statistics_comparisonEnc2Enc.txt", std::ios::app);
  
	int testDataCounter = 0;



	raw_voting_data * testVotingDataRows2 = new raw_voting_data[10];
	raw_voting_data * testVotingDataRows3 = new raw_voting_data[10];

    start = std::chrono::high_resolution_clock::now(); 

	for (int i = 0; i < RAW_VOTING_DATA_LENGTH; i++) {
        testVotingDataRows2[testDataCounter].id[i] = testVotingDataRows3->id[i];
        testVotingDataRows2[testDataCounter].name[i] = testVotingDataRows3->name[i];
        testVotingDataRows2[testDataCounter].sex[i] = testVotingDataRows3->sex[i];
        testVotingDataRows2[testDataCounter].choice[i] = testVotingDataRows3->choice[i];
        testVotingDataRows2[testDataCounter].rights[i] = testVotingDataRows3->rights[i];
	}
    end = std::chrono::high_resolution_clock::now(); 

    elapsed_seconds = end - start; 
    end_time = std::chrono::high_resolution_clock::to_time_t(end);   
    std::cout << "Comparison Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout6 << "Comparison Enc2Enc: elapsed time:" << elapsed_seconds.count() << " s.   \tFinished computation at:"  << std::ctime(&end_time); 
	fout6.close();
	...
	char outputenclave_function_identifier = 'T';
	Enclave2_general_output_function_wrapper(e2_enclave_id, outputenclave_function_identifier);

	//marker4	
	getchar();
	/* Are we attesting, or just spitting out a quote? */
	if ( config.mode == MODE_ATTEST ) {
		do_attestation(e1_enclave_id, &config);
	} else if ( config.mode == MODE_EPID || config.mode == MODE_QUOTE ) {
		do_quote(e1_enclave_id, &config);
	} else {
...
int do_attestation (sgx_enclave_id_t eid, config_t *config)
{
	...
	//MARKER Begin of remote attestation
	verbose=0;
	debug=0;
	ofstream fout2;
	fout2.open("statistics_RemoteAttestation.txt", std::ios::app);
	std::chrono::time_point<std::chrono::system_clock> start, end; 
	start = std::chrono::high_resolution_clock::now(); 
	...
	if ( enclaveTrusted == Trusted ) {
		...
		//DEBUG to cover missing bios update at hand
	...
	//MARKER End of remote Attestation
	end = std::chrono::high_resolution_clock::now(); 

	std::chrono::duration<double> elapsed_seconds = end - start; 
	std::time_t end_time = std::chrono::high_resolution_clock::to_time_t(end);   
	std::cout << "Init RemoteAttestation: elapsed time:" << elapsed_seconds.count() << " s.     \tFinished computation at:"  << std::ctime(&end_time); 
	fout2 << "Init RemoteAttestation: elapsed time:" << elapsed_seconds.count() << " s.     \tFinished computation at:"  << std::ctime(&end_time); 
	fout2.close();
	...
	if ( enclaveTrusted == Trusted || true) {
	...
	/* 
	 * Save MK as variable inside enclave.
	 */
	if (enclaveTrusted == Trusted || true) {
		sgx_status_t statusEnclaveCall, retval, statusKey;
		statusEnclaveCall = enclave_ra_get_key_saved(	
			eid, &retval, &statusKey, ra_ctx,
			SGX_RA_KEY_MK);
		if (statusEnclaveCall != SGX_SUCCESS) {
			enclave_ra_close(eid, &sgxrv, ra_ctx);
			fprintf(stderr, "Error: Save mk inside enclave: %08x\n", status);
			fprintf(fplog, "Error: Save mk inside enclave: %08x\n", status);
			delete msgio;
			return 1;
		}
		if (statusKey != SGX_SUCCESS) {
			enclave_ra_close(eid, &sgxrv, ra_ctx);
			fprintf(stderr, "Error: Save mk inside enclave: %08x\n", status);
			fprintf(fplog, "Error: Save mk inside enclave: %08x\n", status);
			delete msgio;
		...
		fprintf(stderr, "Complete: Save mk inside enclave: \n");
		fprintf(fplog, "Complete: Save mk inside enclave: \n");
	}

	/* 
	 * Receive password inside the verified enclave,
	 * encrypt it using MK and provide to untrusted code.
	 * TODO
	 * Code taken from https://github.com/rodolfoams/sgx-aes-gcm/blob/master/CryptoTestingApp/CryptoTestingApp.cpp
	 */
	char* passwordPointer;
	size_t passwordLengthPointer;
	if (enclaveTrusted == Trusted || true) {
		...
		passwordPointer = encMessage;
		passwordLengthPointer = encMessageLen;
		printf("Encrypting...\n");
		...
		print_hexstring(stdout, &encMessage, encMessageLen+1);
		...
		if (ret != SGX_SUCCESS) {
			enclave_ra_close(eid, &sgxrv, ra_ctx);
			fprintf(stderr, "Error: Decrypting password with mk: %08x\n", status);
			fprintf(fplog, "Error: Decrypting password with mk: %08x\n", status);
			delete msgio;
			return 1;
		}
	...
	//	getchar();
		...
		fprintf(stderr, "Complete: Encrypt password with mk: \n");
		fprintf(fplog, "Complete: Encrypt password with mk: \n");
	}
	
	/* 
	 * Send the connector data and encrypted user credentials to server.
	 * TODO
	 */

	fprintf(stderr, "Sending UserInfo: \n");
	fprintf(fplog, "Sending UserInfo: \n");

	char* myType=             "SGX-CONNECTOR";
	char* myInfoModel=        "InfoModel-Bla";
	char* myUserCredentials=  "UserStuffPassw";

	fprintf(stderr, "FUNCTION CALL: \n");
	fprintf(fplog, "FUNCTION CALL: \n");
	sendMyMessage(msgio, myType, 14);
	sendMyMessage(msgio, myInfoModel, 14);
	sendMyMessage(msgio, myUserCredentials, 20);

/*	char * encryptedPasswordPointer = encryptMyMessage(myUserCredentials);
	char * b64passwordPointer = base64_encode(encryptedPasswordPointer, (int) strlen(encryptedPasswordPointer));
	sendMyMessage(msgio, b64passwordPointer, (int) strlen(b64passwordPointer));
*/

	size_t myMessageSizeEnc;
	char * encryptedPasswordPointer = encryptMyMessage(myUserCredentials, &myMessageSizeEnc);
	printf("\nDEBUG:Output encryp-length: %i\n",(int) myMessageSizeEnc);
	int myMessageSizeIntEnc = (int) myMessageSizeEnc;
	char * b64passwordPointer = base64_encode(encryptedPasswordPointer, strlen(encryptedPasswordPointer)); //+4 strlen gibt nur Anzahl der Zeichen ohne Newline Char
	printf("\nDEBUG:Output base64 Client: %s\n", b64passwordPointer);
	size_t myMessageSize = strlen(b64passwordPointer);
	int myMessageSizeInt = (int) myMessageSize;
	sendMyMessage(msgio, b64passwordPointer, myMessageSizeInt );

		getchar();

	/* 
	 * Close the enclave and free memory.	
	 */
	...
	sgx_misc_attribute_t *attr)
{
	...
	//MARKERTWO Init Management Enlcave Init ME
	if ( stat(filename, &sb) == 0 ){
		sgx_status_t result;

		ofstream fout;
		fout.open("statistics_initManagementEnclave.txt", std::ios::app);
		std::chrono::time_point<std::chrono::system_clock> start, end; 
	
		start = std::chrono::high_resolution_clock::now(); 

		result = sgx_create_enclave(filename, edebug, token, updated, eid, attr);

		end = std::chrono::high_resolution_clock::now(); 

		std::chrono::duration<double> elapsed_seconds = end - start; 
		std::time_t end_time = std::chrono::high_resolution_clock::to_time_t(end);   
		std::cout << "Init ManagementEnclave: elapsed time:" << elapsed_seconds.count() << " s.     \tFinished computation at:"  << std::ctime(&end_time); 
		fout << "Init ManagementEnclave: elapsed time:" << elapsed_seconds.count() << " s.     \tFinished computation at:"  << std::ctime(&end_time); 
		fout.close();
